...
export async function POST(req: Request) {
  ...
  const header = ['#separator:tab', '#html:true', `#deck:${escapeField(title)}`];
  const body = flashcards.map((c) => `${escapeField(c.front)}\t${escapeField(c.back)}`);
  const txt = [...header, ...body].join('\n');

  const filename = `${slug(title)}-anki.txt`;
