...
export function HeartBubble({ className, count = 1 }: { className?: string; count?: number }) {
  return (
    <div className={cn('relative inline-flex items-center gap-1.5 bg-ink text-cream rounded-full px-3 py-1 shadow-sticker', className)}>
...
export function Pushpin({ className, size = 26 }: StickerProps) {
  return (
    <svg viewBox="0 0 32 32" width={size} height={size} className={cn('drop-shadow-[0_2px_3px_rgba(0,0,0,0.25)]', className)} aria-hidden>
      <defs>
        <radialGradient id="pin" cx="35%" cy="35%" r="65%">
          <stop offset="0%" stopColor="#F5F5F5" />
          <stop offset="60%" stopColor="#888" />
          <stop offset="100%" stopColor="#2A2A2A" />
        </radialGradient>
      </defs>
      <circle cx="16" cy="11" r="8" fill="url(#pin)" stroke="#1F1F1F" strokeWidth="0.6" />
      <circle cx="13" cy="8" r="2.5" fill="rgba(255,255,255,0.55)" />
      <path d="M15 18 L17 18 L16.2 28 Z" fill="#1F1F1F" />
    </svg>
  );
}
...
export function ToBeContinued({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        'inline-flex items-center gap-1.5 bg-ink text-cream px-3 py-1 font-elite text-[11px] uppercase tracking-wider',
        'before:content-[""] before:absolute before:left-[-8px] before:top-0 before:bottom-0 before:w-2 before:bg-ink',
        'relative shadow-sticker',
        className,
      )}
...
export function CornerTape({ className, rotate = 45 }: { className?: string; rotate?: number }) {
  return (
    <div
      className={cn('absolute w-20 h-5 bg-tape/80 shadow-sm', className)}
...
export function MemoryTicket({ className }: { className?: string }) {
  return (
    <div className={cn('relative inline-block', className)}>
      <div
        className="px-4 py-1.5 bg-[#EFE4C8] text-ink/80 font-elite text-[11px] uppercase tracking-[0.25em] border border-ink/15"
        style={{
          clipPath:
...
export function Lightbulb({ className, size = 28 }: StickerProps) {
  return (
    <svg viewBox="0 0 32 40" width={size} height={size * 1.25} className={cn('drop-shadow-[0_2px_3px_rgba(0,0,0,0.2)]', className)} aria-hidden>
      <ellipse cx="16" cy="14" rx="10" ry="11" fill="#F2E5B0" stroke="#1F1F1F" strokeWidth="1" />
      <path d="M9 16 Q 16 22, 23 16" fill="none" stroke="#1F1F1F" strokeWidth="0.8" opacity="0.6" />
